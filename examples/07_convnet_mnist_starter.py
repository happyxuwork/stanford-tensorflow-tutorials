...
# clear the checkpoints in the directory: checkpoints/convmnist
OUTPUT_DIR = './checkpoints/convnet_mnist'
if os.path.exists(OUTPUT_DIR):
    import shutil
    shutil.rmtree(OUTPUT_DIR)
    print('remove existing files in directory: {}'.format(OUTPUT_DIR))
# Step 1: Read in data
...
with tf.variable_scope('conv1') as scope:
    ...
    w = tf.get_variable(name='weight', shape=[5, 5, 1, 32],
                        initializer=tf.truncated_normal_initializer())
    # TO DO

    # create biases variable of dimension [32]
    # use tf.constant_initializer(0.0)
    b = tf.get_variable(name='bias', shape=[32],
    ...
    # or you can add conv layer like this:
    # use tf.contrib.layer, higher level API
...
with tf.variable_scope('fc') as scope:
    ...
    w = tf.get_variable('weight', shape=[input_features, 1024],
                        initializer=tf.truncated_normal_initializer())
...
with tf.variable_scope('softmax_linear') as scope:
    ...
    # TO DO
    w = tf.get_variable('weight', shape=[1024, N_CLASSES],
                        initializer=tf.truncated_normal_initializer())
