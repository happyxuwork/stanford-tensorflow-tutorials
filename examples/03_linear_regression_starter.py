...
mse_loss = tf.square(y_p - Y) * 0.5

# Step 6: using gradient descent with learning rate of 0.01 to minimize loss
optimizer = tf.train.GradientDescentOptimizer(0.001).minimize(mse_loss)
	...
	for i in range(100): # run 100 epochs
		...
		for x, y in data:
			...
			# TO DO: write sess.run()
                        _, l = sess.run([optimizer, mse_loss], {X: x, Y: y})
			total_loss += l
